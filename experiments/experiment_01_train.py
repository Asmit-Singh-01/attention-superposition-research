...
class MultiTaskDataset(Dataset):
    def __init__(self, size):
        ...
        # Task 2: Parity (odd -> 1, even -> 0)
        number_of_ones = (self.inputs == 1).sum(dim=1)
        self.parity_targets = (number_of_ones % 2).long()
...
class MultiTaskTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        hidden = hidden + self.transformer.position_embedding(positions)
        ...
        parity_logits = self.parity_head(pooled)

        return (copy_logits, parity_logits)
...
def evaluate(model, loader):
    ...
    with torch.no_grad():
        for (x, copy_target, parity_target) in loader:
            x = x.to(DEVICE)
            copy_target = copy_target.to(DEVICE)
            parity_target = parity_target.to(DEVICE)

            copy_logits, parity_logits = model(x)
            
            copy_predictions = copy_logits.argmax(dim=1)
            parity_predictions = parity_logits.argmax(dim=1)

            copy_correct += (copy_predictions == copy_target).sum().item()
            parity_correct += (parity_predictions == parity_target).sum().item()
            
            total += x.size(0)

    copy_accuracy = copy_correct / total
    parity_accuracy = parity_correct / total

    return (copy_accuracy, parity_accuracy)
...
def main():
    ...
    train_dataset = MultiTaskDataset(TRAIN_SIZE)
    test_dataset = MultiTaskDataset(TEST_SIZE)
    ...
    for epoch in range(EPOCHS):
        ...
        for (x, copy_target, parity_target) in train_loader:
            x = x.to(DEVICE)
            copy_target = copy_target.to(DEVICE)
            parity_target = parity_target.to(DEVICE)
        ...
        copy_accuracy, parity_accuracy = evaluate(model, test_loader)
        test_score = (copy_accuracy + parity_accuracy) / 2
    ...
    model_path = "models/experiment_01/multitask_transformer.pt"
    torch.save(model.state_dict(), model_path)

    final_copy, final_parity = evaluate(model, test_loader)

    print("\n" + "=" * 60)
    print("FINAL UNSEEN-DATA RESULTS")
    print("=" * 60)
    print(f"Copy accuracy:   {final_copy:.4f}")
